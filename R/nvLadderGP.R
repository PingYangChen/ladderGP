...
################################################################################
### Naive Ladder GP: Independence across stages
################################################################################
# y: vector
# x: matrix
# z: integer vector
nvLadderFit <- function(yList, xList, 
  ...
  mdl$cputime <- cputime
  cat(sprintf("NaiveLadderGP FIT CPU time: %.2f seconds.\n", cputime))
  return(mdl)
}


nvLadderPred <- function(gpMdl, x0List, y0listTrue = NULL, ei_alpha = 0.5, min_y = NULL) {
